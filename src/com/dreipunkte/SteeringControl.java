package com.dreipunkte;

import lejos.hardware.port.MotorPort;

public class SteeringControl {
	public static final ExtendedEV3LargeRegulatedMotor motorRight = new ExtendedEV3LargeRegulatedMotor(MotorPort.C);
	public static final ExtendedEV3LargeRegulatedMotor motorLeft = new ExtendedEV3LargeRegulatedMotor(MotorPort.B);
	
	
	private static int radius = 14; // 13cm
	private static double maxMotorArac = 5.969; // 5.969 for 360 degree motor spinn
	private static double motorRadius = 0.95; // 0.95 cm
	private static float speed = 1000;
	
	private static double realMaxDegree = 56.0;
	
	public static void goForward() {
		motorRight.setSpeed(speed);
		motorLeft.setSpeed(speed);
		
		motorRight.forward();
		motorLeft.forward();
	}
	
	public static void stop() {
		motorRight.stop();
		motorLeft.stop();
	}

	public static void turnLeft(int angle) {
		turn(motorRight, motorLeft, angle);
	}
	
	public static void turnRight(int angle) {
		turn(motorLeft, motorRight, angle);
	}
	
	private static void turn(ExtendedEV3LargeRegulatedMotor activeMotor, ExtendedEV3LargeRegulatedMotor passiveMotor, int angle) {
		passiveMotor.stop(true);
		activeMotor.stop(true);
		
		MotorState lastActive = activeMotor.getLastState();
		MotorState lastPassive = passiveMotor.getLastState();
		
		// b = ((PI * r) / 180 degree) * alpha
		
		// Small degree correction
		int correctedAngle = correctedAngle(angle);
				
		double b =  (double) (((Math.PI * radius) / 180) * correctedAngle); 
		
		double neededRotationFactor = b / maxMotorArac;
		
		double neededRotation = neededRotationFactor * maxMotorArac;
		
		// alpha = b / ( (PI * r) / 180 degree );
		
		int motorAngleToTurn = (int) Math.round( neededRotation / ((Math.PI * motorRadius) / 180) );
		
		passiveMotor.flt(true);
		
		activeMotor.rotate(motorAngleToTurn);
		
		activeMotor.doNewState(lastActive, true);
		
		passiveMotor.doNewState(lastPassive, true);
	}
	
	private static int correctedAngle(int angle) {
		return (int) Math.round((realMaxDegree / 90.0) * angle);
	}
}
